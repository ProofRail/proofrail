#!/usr/bin/env python3
"""Export a portable independent Silver verification package (v0.2.1).
...
v0.2.1 adds package_format_version, profile_compatibility, inputs, and
path_map fields to the package manifest. Structural layout is unchanged
from v0.1.0.
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Export a portable independent Silver verification package (v0.2.1)"
    )
    ...
    # 5. Generate package manifest — derive paths from actual export layout
    bronze_rel = str(bronze_root.relative_to(repo_root))  # e.g. "demos/composed-bronze-demo-001"
    silver_rel = str(silver_root.relative_to(repo_root))  # e.g. "demos/silver-demo-001"

    subset_prefix = "source-repo-subset"
    assertion_rel = f"{subset_prefix}/{silver_rel}/runtime/silver-signed-bundle-assertion-v0.1.0.yaml"
    trust_policy_rel = f"{subset_prefix}/{silver_rel}/runtime/verifier-b/trust-policy.yaml"
    revocation_list_rel = f"{subset_prefix}/{silver_rel}/runtime/verifier-b/revocation-list.yaml"
    bundle_manifest_rel = f"{subset_prefix}/{bronze_rel}/evidence-bundle-manifest-v0.1.3.yaml"
    bronze_root_rel = f"{subset_prefix}/{bronze_rel}"
    ...
    manifest = {
        ...
        "generated_by": "tools/silver/export_independent_verification_package_v0_2_1.py",
        ...
        "inputs": {
            "signed_bundle_assertion": assertion_rel,
            "trust_policy": trust_policy_rel,
            "revocation_list": revocation_list_rel,
            "bronze_bundle_manifest": bundle_manifest_rel,
            "evidence_root": bronze_root_rel,
        },
        "verifier": {
        ...
        "paths": {
            "signed_assertion": assertion_rel,
            "trust_policy": trust_policy_rel,
            "revocation_list": revocation_list_rel,
            "bundle_manifest": bundle_manifest_rel,
            "bronze_package_root": bronze_root_rel,
            "report_output": "verification-report.json",
        },
        "path_map": {
            "original_repo_root": repo_root.name,
            "package_repo_root": subset_prefix,
        },
        "limitations": [
