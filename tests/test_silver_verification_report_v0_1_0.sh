...
VERIFIER_ARGS="$ASSERTION $TRUST_POLICY --silver-root $SILVER_ROOT --bronze-package-root $BRONZE_ROOT --revocation-list $REVOCATION_LIST --report $REPORT"
...
echo "=== Step 8: Verify assertion with --report (happy path) ==="
python3 tools/silver/verify_signed_bundle_assertion_v0_1_0.py $VERIFIER_ARGS

echo "=== Step 9: Validate pass report structure ==="
python3 tools/silver/validate_silver_verification_report_v0_1_0.py "$REPORT"

echo "=== Step 10: Inline checks on pass report ==="
python3 - "$REPORT" <<'PY'
import json
import sys
...
if failed:
    print("FAIL: pass report inline checks failed")
    for name in failed:
        print(f"  - {name}")
    sys.exit(1)

print("PASS: pass report inline checks valid")
PY

echo "=== Step 11: Generate failure report — revoked assertion ID ==="
python3 tools/silver/generate_demo_revocation_list_v0_1_0.py "$SILVER_ROOT" --force \
  --revoke-assertion proofrail-silver-demo-001 \
  --reason "demo assertion revocation for report test"

FAIL_OUTPUT=$(python3 tools/silver/verify_signed_bundle_assertion_v0_1_0.py $VERIFIER_ARGS 2>&1 || true)

if echo "$FAIL_OUTPUT" | grep -q "assertion revoked"; then
    echo "PASS: verifier correctly rejected revoked assertion"
else
    echo "FAIL: expected 'assertion revoked' in verifier output"
    echo "  Output: $FAIL_OUTPUT"
    exit 1
fi

echo "=== Step 12: Validate failure report structure ==="
python3 tools/silver/validate_silver_verification_report_v0_1_0.py "$REPORT"

echo "=== Step 13: Inline checks on failure report ==="
python3 - "$REPORT" <<'PY'
import json
import sys
from pathlib import Path
...
if failed:
    print("FAIL: failure report inline checks failed")
    for name in failed:
        print(f"  - {name}")
    sys.exit(1)

print("PASS: failure report inline checks valid")
PY

# Restore empty revocation list for clean state
python3 tools/silver/generate_demo_revocation_list_v0_1_0.py "$SILVER_ROOT" --force

echo "=== PASS: Silver verification report v0.1.0 regression fixture valid ==="
