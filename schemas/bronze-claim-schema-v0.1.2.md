# ProofRail Bronze Claim Schema v0.1.2

**Version:** v0.1.2
**Date:** 2026-06-14
**Status:** Draft / Demo-informed schema
**Derived from:** v0.1.1 + evidence checksum extension
...
Version v0.1.2 extends v0.1.1 with an optional `evidence_checksums` mapping, enabling post-generation integrity verification of evidence files via SHA-256 digests computed over raw file bytes.
...
The purpose of v0.1.2 is not to create a production certification regime. It is to stabilize the schema semantics needed for early Bronze-style evidence packages and local validation.
...
v0.1.2 supports two claim types:
...
A v0.1.2 Bronze claim must include:
...
| ------------------- | -------: | ------------------------------------------------------------ |
| `spec_version`      |      Yes | Schema version. Must be `v0.1.2`.                            |
...
| Field                 | Required | Meaning                                                        |
| --------------------- | -------: | -------------------------------------------------------------- |
| `evidence_checksums`  | Optional | SHA-256 checksums of evidence files for integrity verification. |
...
A v0.1.2 Bronze claim must include the following control booleans:
...
For a passing Bronze v0.1.2 claim, all listed controls must be `true`.
...
v0.1.2 includes `control_details` to prevent overclaiming.
...
A v0.1.2 Bronze claim must include evidence file references.
...
## 10A. Evidence Checksums

v0.1.2 adds an optional but recommended `evidence_checksums` top-level mapping for post-generation evidence integrity verification.

```yaml
evidence_checksums:
  "docs/architecture.md": "sha256:abcdef0123456789..."
  "evidence/audit-sample.jsonl": "sha256:fedcba9876543210..."
```

### 10A.1 Field specification

| Property | Requirement | Meaning |
| -------- | ----------: | ------- |
| Key      |    Required | Evidence file path, relative to the claim package root. Must be a non-empty string. |
| Value    |    Required | SHA-256 digest in the format `sha256:<64 hex chars>`, computed over the raw bytes of the file. |

### 10A.2 Scope

Generators should emit checksums for all evidence files referenced in the `evidence` section that exist on disk at generation time.

The claim YAML file's own checksum is NOT included in `evidence_checksums`.

### 10A.3 Computation rule
...
A verifier tool should:

1. Load the claim YAML and extract `evidence_checksums`.
2. For each entry, resolve the path relative to the package root.
3. Read the file as raw bytes and compute SHA-256.
4. Fail if any file is missing or its checksum does not match.
5. Pass if all checksums match.

### 10A.5 Structural validation

A structural validator should check the shape of `evidence_checksums` when present:

* The field must be a mapping.
* Each key must be a non-empty string.
* Each value must match `sha256:[0-9a-fA-F]{64}`.

The structural validator does NOT recompute checksums.

---

## 11. Validation Section

A v0.1.2 Bronze claim must include a validation section.
...
v0.1.2 requires explicit limitations.
...
## 13. Minimal Valid v0.1.2 Example

```yaml
spec_version: "v0.1.2"
claim_type: "proofrail.bronze.composed"
claim_id: "proofrail-demo-001b-20260614"
...
evidence_checksums:
  "docs/architecture.md": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "docs/runbook.md": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "evidence/protected_actuator_set.json": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "evidence/bypass-test-results.md": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "evidence/emergency-stop-test.md": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "evidence/rate-limit-test.md": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "evidence/audit-sample.jsonl": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "evidence/latency-benchmark.md": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
  "evidence/agentgateway-log-sample.txt": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"

validation:
  type: "self-attested-demo"
  validator: "ProofRail Bronze claim validator v0.1.2 structural check"
...
A local v0.1.2 validator should perform structural validation only.
...
* if `evidence_checksums` is present: it must be a mapping, each key must be a non-empty string, each value must match `sha256:[0-9a-fA-F]{64}`
...
* optional `evidence_checksums` top-level mapping for evidence integrity verification
* SHA-256 checksums computed over raw file bytes
* structural validation of checksum field shape when present
* standalone evidence verifier tool for checksum recomputation and comparison
...
But v0.1.2 requires the claim to state which mechanism was actually observed.
