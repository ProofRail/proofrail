# Silver Profile Conformance Report Schema v0.2.0

## 1. Purpose

This schema defines the structure of a Silver Profile Conformance Report — the output of the Silver profile validator. The conformance report records whether a Silver verification result meets the requirements of the Silver Relying-Party Profile v0.2.0.

## 2. Relationship to Silver Profile v0.2.0

The Silver profile validator (`tools/silver/validate_silver_profile_v0_2_0.py`) implements this schema. This document is the normative definition; the validator is the reference implementation.

## 3. Relationship to Silver Verification Report v0.1.0
...
|-------|------|-------------|
| `conformance_report_version` | string | Must be `"v0.2.0"` |
...
| `profile_version` | string | Must be `"v0.2.0"` |
| `profile_mode` | string | `"silver.base"` or `"silver.independent"` |
...
| `package_manifest` | string or null | Path to the package manifest YAML (null for silver.base) |
...
| `profile_requirements_satisfied_with_revocation_warning` | pass | All requirements met but revocation was not performed (silver.base only) |
...
The `not_applicable` status is used only for `independent_package_manifest_valid` in `silver.base` mode.

## 10. Warnings

An array of human-readable warning strings. May be empty.

The primary use case is the revocation warning for `silver.base` when `revocation_check.performed == false`:
...
2. `conformance_report_version` is `"v0.2.0"`.
3. `conformance_report_type` is `"proofrail.silver.profile_conformance_report"`.
4. `profile.profile_mode` is `"silver.base"` or `"silver.independent"`.
