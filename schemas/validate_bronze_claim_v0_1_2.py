#!/usr/bin/env python3
"""Local structural validator for ProofRail Bronze Claim Schema v0.1.2.
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: validate_bronze_claim_v0_1_2.py <claim.yaml>", file=sys.stderr)
        return 2
    ...
    if claim.get("spec_version") != "v0.1.2":
        err(errors, "spec_version must be v0.1.2")
    ...
    if not isinstance(controls, dict):
        ...
        for key in REQUIRED_CONTROLS:
            if key not in controls:
                ...
                err(errors, f"controls.{key} must be true for a passing Bronze v0.1.2 claim")
    ...
    # Optional evidence_checksums validation (structural only, no recomputation)
    checksums = claim.get("evidence_checksums")
    if checksums is not None:
        if not isinstance(checksums, dict):
            err(errors, "evidence_checksums must be a mapping if present")
        else:
            for ck, cv in checksums.items():
                if not isinstance(ck, str) or not ck:
                    err(errors, "evidence_checksums keys must be non-empty strings")
                if not isinstance(cv, str) or not re.fullmatch(r"sha256:[0-9a-fA-F]{64}", cv):
                    err(errors, f"evidence_checksums value for '{ck}' must be sha256:<64 hex chars>")
    ...
    print(f"PASS: {path} is structurally valid for ProofRail Bronze Claim Schema v0.1.2")
    return 0
