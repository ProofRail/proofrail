...
ProofRail v0.3.6 introduces a deterministic local Silver evidence primitive — a hand-authored control pack declaring a closed-vocabulary protected action catalog and a closed crosswalk of those actions to ProofRail-internal evidence artifacts under conservative claim verbs — paired with a byte-for-byte re-derivable conformance report and a 2-subject manifest. v0.3.6 introduces no new signature scheme, trust authority, runtime substrate, or evaluation of any specific upstream Silver evidence. It introduces a single, deterministic, local Control Crosswalk + Protected Action Catalog package with two hash-anchored subjects in fixed order.

| Layer | Artifact | Version | Primary role | Main file/schema |
|---|---:|---:|---|---:|
| Silver | Silver Control Crosswalk + Protected Action Catalog | v0.1.0 | Hand-authored local control pack declaring a closed protected action catalog and a closed crosswalk to ProofRail-internal artifact_type values under conservative `may_inform` / `may_evidence` / `may_support` claim verbs | `schemas/silver-control-crosswalk-protected-action-catalog-v0.1.0.md` |
| Silver | Silver Control Crosswalk + Protected Action Catalog Manifest | v0.1.0 | 2-subject SHA-256 hash anchor in fixed order (control pack / conformance report) with `proofrail_release: silver.control_crosswalk.v0.3.6` and a cross-anchored `control_pack_id` | `schemas/silver-control-crosswalk-protected-action-catalog-manifest-v0.1.0.md` |
| Silver | Silver Control Crosswalk + Protected Action Catalog Conformance Report | v0.1.0 | Re-derivable conformance record with 24 pass entries, one per approved verifier check; canonical-JSON byte image depends only on the control pack | `schemas/silver-control-crosswalk-protected-action-catalog-conformance-report-v0.1.0.md` |
| Doc | Silver Control Crosswalk + Protected Action Catalog | v0.3.6 | Release narrative document | `docs/silver/silver-control-crosswalk-protected-action-catalog-v0.3.6.md` |

The runner (`tools/silver/build_silver_control_crosswalk_protected_action_catalog_v0_1_0.py`) runs five Phase A preflight checks against `--input-pack` (each emitting a single, distinct, runner-only refusal reason: `runner_input_path_missing`, `runner_input_path_forbidden`, `runner_input_file_missing`, `runner_input_read_failed`, `runner_input_json_invalid`); Phase A never touches the output directory or staging sibling. The runner then stages every byte under `<output-dir>.staging.<pid>`, byte-copies the control pack to `<staging>/control-pack.json`, deterministically re-derives the 24-entry conformance report as canonical JSON bytes, writes the 2-subject manifest with a cross-anchored `control_pack_id`, runs self-validation against the staging directory **before** the atomic `os.replace()`, and (on `--self-validate` failure) **relays the v0.3.6 verifier's OWN failure reason UNCHANGED** — the runner never wraps a verifier failure in a sixth runner-only code. A refused or self-validation-failed run leaves no final directory and no staging sibling, so the Make target is safely repeatable.

The verifier (`tools/silver/verify_silver_control_crosswalk_protected_action_catalog_v0_1_0.py`) is non-masking and emits 24 stable failure reasons across 25 ordered execution steps: `control_pack_manifest_invalid`, `control_pack_not_object`, `control_pack_schema_invalid`, `control_pack_profile_unsupported`, `control_pack_identity_invalid`, `protected_action_catalog_invalid`, `protected_action_entry_invalid`, `protected_action_identifier_invalid`, `protected_action_scope_invalid`, `protected_action_authority_invalid`, `protected_action_risk_boundary_invalid`, `control_crosswalk_invalid`, `crosswalk_entry_invalid`, `catalog_crosswalk_consistency_invalid`, `proofrail_artifact_reference_invalid`, `evidence_relationship_invalid`, `control_concept_reference_invalid`, `control_objective_invalid`, `control_claim_invalid`, `control_limitation_invalid`, `dependency_reference_invalid`, `version_binding_invalid`, `non_claims_missing`, `prohibited_compliance_claim_present`. All 23 dedicated structural checks run BEFORE the bundled conformance report is parsed and byte-compared against the deterministic re-derivation, so dedicated subject [0] failures are never masked behind a downstream report-disagreement reason. Bundled-report byte disagreement folds back to `control_pack_manifest_invalid` (the bundled report does not describe a passing verification of this control pack); no twenty-fifth public reason is introduced. The recursive overclaim guard scans every string value outside `scope_limitations`, `non_claims`, and `relying_party.contact` for 32 forbidden positive tokens including external framework names (`soc 2`, `iso 27001`, `nist 800-53`, `pci dss`, `hipaa`), control-effectiveness claims (`control design effectiveness`, `control operating effectiveness`), `audit-ready`, regulator/auditor approval tokens, `production approved`, `production-ready`, `legally enforceable`, `transferred trust`, `runtime truth`, `gold accepted`, and `gold certified`. The five runner-only refusal codes are deliberately distinct and never emitted by the verifier.

A Silver Control Crosswalk + Protected Action Catalog package is not regulator approval, auditor approval, third-party endorsement, certification, a SOC 2 / ISO 27001 / NIST 800-53 / PCI DSS / HIPAA mapping, a control design or operating effectiveness opinion, a Gold artifact, an adjudication, an evidence-evaluation result, legally enforceable, or production authorization. The packaged conformance report describes only that the control pack satisfies the 24 ordered structural checks; it does not describe a substantive review of any specific upstream Silver evidence in this release. v0.3.6 packages a hand-authored protected action catalog and crosswalk alongside a re-derivable structural-conformance report. It does not evaluate any specific upstream composed-gateway evidence, trace-binding, acceptance record, revocation-challenge drill, handoff, or other Silver evidence against the catalog, and it does not extend the substance of any earlier-release Silver evidence.

## v0.3.7 Registry Lite

ProofRail v0.3.7 introduces a deterministic local Silver evidence primitive — a hand-authored registry-lite document declaring at least one entry for each of the six Silver entity roles (issuer, verifier, relying party, policy authority, revocation source, protected action authority) under a closed enum / status / scope / role-specific vocabulary — paired with a byte-for-byte re-derivable conformance report and a 2-subject manifest. v0.3.7 introduces no new signature scheme, trust authority, runtime substrate, certificate authority, identity-proofing record, federation registry, or evaluation of any specific upstream Silver evidence against the registry. It introduces a single, deterministic, local Registry Lite package with two hash-anchored subjects in fixed order.

| Layer | Artifact | Version | Primary role | Main file/schema |
|---|---:|---:|---|---:|
| Silver | Silver Registry Lite | v0.1.0 | Hand-authored local registry-lite document declaring at least one entry for each of the six Silver entity roles under a closed enum / status / scope / role-specific vocabulary | `schemas/silver-registry-lite-v0.1.0.md` |
| Silver | Silver Registry Lite Manifest | v0.1.0 | 2-subject SHA-256 hash anchor in fixed order (registry body / conformance report) with `proofrail_release: silver.registry_lite.v0.3.7` and a cross-anchored `registry_id` | `schemas/silver-registry-lite-manifest-v0.1.0.md` |
| Silver | Silver Registry Lite Conformance Report | v0.1.0 | Re-derivable conformance record with 24 pass entries, one per approved verifier check; canonical-JSON byte image depends only on the registry body | `schemas/silver-registry-lite-conformance-report-v0.1.0.md` |
| Doc | Silver Registry Lite | v0.3.7 | Release narrative document | `docs/silver/silver-registry-lite-v0.3.7.md` |

The runner (`tools/silver/build_silver_registry_lite_v0_1_0.py`) runs five Phase A preflight checks against `--input-registry` (each emitting a single, distinct, runner-only refusal reason: `runner_input_path_missing`, `runner_input_path_forbidden`, `runner_input_file_missing`, `runner_input_read_failed`, `runner_input_json_invalid`); Phase A never touches the output directory or staging sibling. The runner then stages every byte under `<output-dir>.staging.<pid>`, byte-copies the registry body to `<staging>/registry-lite.json`, deterministically re-derives the 24-entry conformance report as canonical JSON bytes (`sort_keys=True, separators=(",",":")` with trailing newline), writes the 2-subject manifest with a cross-anchored `registry_id`, runs self-validation against the staging directory **before** the atomic `os.replace()`, and (on `--self-validate` failure) **relays the v0.3.7 verifier's OWN failure reason UNCHANGED** — the runner never wraps a verifier failure in a sixth runner-only code. A refused or self-validation-failed run leaves no final directory and no staging sibling, so the Make target is safely repeatable.

The verifier (`tools/silver/verify_silver_registry_lite_v0_1_0.py`) is non-masking and emits 24 stable failure reasons across the ordered execution sequence: `registry_manifest_invalid`, `registry_not_object`, `registry_schema_invalid`, `registry_profile_unsupported`, `registry_identity_invalid`, `registry_authority_invalid`, `registry_entity_set_invalid`, `registry_entity_entry_invalid`, `registry_entity_identifier_invalid`, `registry_role_invalid`, `registry_status_invalid`, `registry_effective_period_invalid`, `registry_key_reference_invalid`, `registry_key_binding_invalid`, `issuer_entry_invalid`, `verifier_entry_invalid`, `relying_party_entry_invalid`, `policy_authority_entry_invalid`, `revocation_source_entry_invalid`, `protected_action_authority_entry_invalid`, `trust_relationship_invalid`, `version_binding_invalid`, `non_claims_missing`, `prohibited_registry_claim_present`. The 23 dedicated structural checks run BEFORE the bundled conformance report is parsed and byte-compared against the deterministic re-derivation, so dedicated subject [0] failures are never masked behind a downstream report-disagreement reason. Bundled-report byte disagreement on an otherwise valid registry folds back to `registry_manifest_invalid` (the bundled report does not describe a passing verification of this registry); no twenty-fifth public reason is introduced. Entity-entry checks fire in the documented order so `registry_entity_identifier_invalid` precedes `registry_role_invalid` / `registry_status_invalid` / `registry_effective_period_invalid` within the entity-entry loop, `registry_key_reference_invalid` precedes `registry_key_binding_invalid`, and role-specific reasons 15..20 fire only after the base entity-entry structural checks pass for that entry. The recursive overclaim guard scans every string value outside `non_claims` and `scope_limitations` for 36 forbidden positive tokens including `production PKI`, `certificate authority`, `certification authority`, `legal identity`, `identity proofing`, `federated trust`, `trust federation`, `production trust registry`, `certified`, `compliance`, `legally enforceable`, `regulator approved`, `auditor approved`, `audit ready`, `operating effectiveness`, `runtime truth`, `transferred trust`, `gold governed reliance`, `gold accepted`, and `gold certified`. The five runner-only refusal codes are deliberately distinct and never emitted by the verifier.

A Silver Registry Lite package is not production PKI, a certificate authority, a certification authority, a legal identity registry, an identity-proofing record, a federation registry or trust federation, regulator approval, auditor approval, third-party endorsement, certification, a Gold artifact, an adjudication, an evidence-evaluation result, legally enforceable, a transfer of reliance, or production authorization. The packaged conformance report describes only that the registry body satisfies the 24 ordered structural checks; it does not describe a substantive review of any specific upstream Silver evidence in this release. v0.3.7 packages a hand-authored local registry declaring at least one entry per each of the six Silver entity roles alongside a re-derivable structural-conformance report. It does not evaluate any specific upstream Silver evidence (composed-gateway, trace-binding, acceptance record, revocation-challenge drill, acceptance-handoff, handoff-inspection, control-crosswalk, etc.) against the registry, it does not issue / transfer / accept any reliance instrument against the registry, and it does not extend the substance of any earlier-release Silver evidence.
