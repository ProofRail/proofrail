...
**Status:** Released — ProofRail v0.4.3 — narrow incremental Gold release
...
├── challenge-lifecycle-records.json                          (subject [5])
...
- `subjects[5]`: role `challenge_lifecycle_records`, path `challenge-lifecycle-records.json`
...
| R45 | `gold_challenge_lifecycle_report_schema_invalid` | top-level shape, `document_type`, `schema_version`, identifier grammar, required field set; hex-shape check over the four bare-lowercase-hex SHA-256 fields (`source_records_sha256`, `source_policy_evaluation_report_sha256`, `source_decision_report_sha256`, `report_fingerprint`) |
| R46 | `gold_challenge_lifecycle_report_binding_invalid` | cross-anchors between lifecycle report and manifest / package / records body (`package_id`, `governed_reliance_demo_id`, `challenge_lifecycle_record_set_id`, `challenge_lifecycle_report_id`, `policy_evaluation_report_id`, `source_records_sha256`, `source_policy_evaluation_report_sha256`, `source_decision_report_sha256`) |
| R47 | `gold_challenge_lifecycle_projection_invalid` | per-row projection drift in `lifecycle_rows[]` against the published records body (row count, row order, per-row scalar mismatches over `row_id`, `lifecycle_id`, `target_decision_id`, `target_decision_row_id`, `current_status`, `is_terminal`, `event_count`, `first_event_id`, `final_event_id`, `final_event_timestamp`, `lifecycle_fingerprint`); plus a deliberate post-R48 sub-check that re-derives the top-level `report_fingerprint` (canonical-JSON SHA-256 over the report body excluding the fingerprint field) and routes a mismatch to R47 |
| R48 | `gold_challenge_lifecycle_summary_invalid` | `coverage_summary` block: closed five-field set `lifecycle_record_count`, `lifecycle_event_count`, `open_lifecycle_count`, `terminal_lifecycle_count`, `status_value_count` (6-key closed map over `filed`, `acknowledged`, `under_review`, `resolved_locally`, `superseded`, `withdrawn`); each value re-derived from `lifecycle_rows[]` |

R47 and R48 are split: row-level projection drift routes to R47, and any drift inside `coverage_summary` routes to R48. After R48 passes, the verifier additionally re-derives the top-level `report_fingerprint` and routes a mismatch back to R47; this post-R48 R47 sub-check is a deliberate non-monotonic placement so that row-level projection (R47) and coverage-summary (R48) violations remain reachable on unmodified-fingerprint inputs. The split keeps both reasons independently reachable.
...
- Within records-body checks the natural ordering is: not-object (R39) → schema-shape (R40) → binding (R41) → per-event validity (R42) → sequence/graph validity (R43) → deliberate post-R43 R41 sub-check (per-record `lifecycle_fingerprint` re-derivation). The post-R43 R41 sub-check is intentionally non-monotonic in the public-token integer suffix; it preserves reachability of R42 and R43 on unmodified-fingerprint inputs and routes a fingerprint mismatch under the existing R41 public token without introducing a new reason.
- Within lifecycle-report checks the natural ordering is: not-object (R44) → schema-shape (R45) → binding (R46) → projection (R47) → summary (R48) → deliberate post-R48 R47 sub-check (top-level `report_fingerprint` re-derivation). The post-R48 R47 sub-check is intentionally non-monotonic; it preserves reachability of R47 row-level projection and R48 coverage-summary checks on unmodified-fingerprint inputs and routes a fingerprint mismatch under the existing R47 public token without introducing a new reason.
...
- 6 `event_basis` values: `challenge_record`, `acknowledgement_record`, `review_update`, `local_resolution_record`, `supersession_record`, `withdrawal_record`.
- 4 `actor_role` values: `relying_party`, `policy_authority`, `reviewer`, `system_recorder`.
- 4 `lifecycle_effect` values: `challenge_open`, `local_resolution_recorded`, `challenge_withdrawn`, `challenge_superseded`.

Each event entry MUST contain a `(event_status, event_basis)` pair drawn from a closed pair table; pair violations are routed to R42. The `(event_status, lifecycle_effect)` pair table is enforced only when `lifecycle_effect` is present: `filed → challenge_open`, `resolved_locally → local_resolution_recorded`, `withdrawn → challenge_withdrawn`, `superseded → challenge_superseded`; `acknowledged` and `under_review` events MUST NOT carry a `lifecycle_effect`. Terminal record-level statuses each require a record-level ref field (R42): `resolved_locally → local_resolution_ref`, `withdrawn → withdrawal_record_ref`, `superseded → superseding_decision_id`. The non-terminal statuses (`filed`, `acknowledged`, `under_review`) carry no record-level ref requirement.

The records-body schema additionally enforces a closed status-transition graph (R43) covering the nine permitted transitions: `filed → acknowledged`, `filed → withdrawn`, `filed → superseded`, `acknowledged → under_review`, `acknowledged → withdrawn`, `acknowledged → superseded`, `under_review → resolved_locally`, `under_review → withdrawn`, `under_review → superseded`. Any unlisted transition routes to R43.
...
`verify-gold-challenge-lifecycle-lite-v0-4-3` runs the dedicated regression test (`tests/test_gold_challenge_lifecycle_lite_v0_4_3.sh`). v0.4.3 shipped with 99 ordered cases:
...
The v0.4.3.1 corrective harness raises the regression to 101 ordered cases by adding two cases that exercise the deliberate non-monotonic fingerprint sub-checks introduced by the v0.4.3.1 verifier correction: rt4 mutates `lifecycle_records[0].lifecycle_fingerprint` (first hex digit flipped, with pre-mutation distinctness assertion) and expects R41; sup06 mutates the top-level `report_fingerprint` under the same pattern and expects R47. The 99/99 reachability surface from the v0.4.3 release is preserved verbatim; the two new cases are appended within the existing categories (runtime-scalar 4 → 5, supplementals 5 → 6). The v0.4.3 release narrative continues to report 99/99 as a historical fact; the 101/101 surface is the v0.4.3.1 corrective harness.
...
The allowlist is intentionally tiny: it carries forward only the five inherited v0.4.1 `coverage_summary` enum-presence data-field names (`decision_statuses_present`, `policy_decisions_present`, `protected_actions_present`, `registry_roles_present`, `scenario_types_present`) which the v0.4.3 build tool inherits indirectly via chained subprocess delegation. No v0.4.3-introduced data-field names appear in the allowlist; v0.4.3 coverage-summary names (`lifecycle_record_count`, `lifecycle_event_count`, `open_lifecycle_count`, `terminal_lifecycle_count`, `status_value_count`) do not match the suffix filter that the gate uses to identify reason-shaped tokens and therefore require no allowlist entry. The closed lifecycle-status keys `filed`, `acknowledged`, `under_review`, `resolved_locally`, `superseded`, `withdrawn` appear inside the `status_value_count` map as data values, not as reason names. The allowlist does not admit semantic synonyms, does not admit paraphrases of failure modes, and does not admit pattern-shaped entries.
...
- The v0.4.3 release does not introduce a new Gold tier; it is a narrow incremental v0.4.x hardening of the challenge-lifecycle surface.
