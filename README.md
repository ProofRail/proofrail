...
ProofRail v0.3.7 introduces the **Silver Registry Lite**: a deterministic local Silver evidence primitive — a hand-authored *registry* declaring at least one entry for each of the six Silver entity roles (issuer, verifier, relying party, policy authority, revocation source, protected action authority), optional trust relationships between those roles under a closed verb vocabulary, and a closed set of upstream Silver version bindings — paired with a byte-for-byte re-derivable *conformance report* inside a single 2-subject packaged manifest. It adds:

- three v0.1.0 schemas (`schemas/silver-registry-lite-v0.1.0.md`, `schemas/silver-registry-lite-manifest-v0.1.0.md`, `schemas/silver-registry-lite-conformance-report-v0.1.0.md`) declaring a closed enum surface (entity `role`, entity `status`, `registry_scope`, `release_binding`, key-reference `algorithm` and `key_reference_type`, key-binding `binding_purpose`, role-specific blocks for issuer / verifier / relying party / policy authority / revocation source / protected action authority, trust-relationship `relationship_verb`, and a closed Silver `upstream_id` / `upstream_version` set for version bindings drawn from v0.2.1..v0.3.6) and a 2-subject manifest layout with `proofrail_release: silver.registry_lite.v0.3.7`;
- a committed fixture set (`fixtures/silver-registry-lite-v0.3.7/`) with one canonical passing registry (one entry per each of the six role types; empty `trust_relationships`; non-empty `version_bindings`; the issuer entry exercises `key_references[]` with `key_reference_type: local_fingerprint` and `key_bindings[]` with `binding_purpose: issue_evidence`) plus two additional passing variants exercising non-empty `trust_relationships[]` across all six closed `relationship_verb` values and a full revocation-source entry with `source_type: signed_revocation_record`, `key_reference_type: local_pem_path`, and `binding_purpose: sign_revocation` — every fixture is locally re-derivable into a passing conformance report;
- a runner (`tools/silver/build_silver_registry_lite_v0_1_0.py`) that performs five distinct preflight checks under five stable runner-only refusal codes (`runner_input_path_missing`, `runner_input_path_forbidden`, `runner_input_file_missing`, `runner_input_read_failed`, `runner_input_json_invalid`), byte-copies the registry body into a deterministic staging directory, deterministically re-derives a 24-entry conformance report whose canonical-JSON byte image depends only on the registry body, writes a 2-subject manifest with a cross-anchored `registry_id`, and atomically `os.replace()`s the staging directory into place only after staging build and (optional) self-validation succeed; on `--self-validate` failure the runner relays the v0.3.7 verifier's own failure code UNCHANGED with **no** sixth runner-only wrapper code;
- a verifier (`tools/silver/verify_silver_registry_lite_v0_1_0.py`) with 24 stable failure reasons across 24 ordered structural checks plus a post-structural conformance-report re-derivation, enforcing non-masking ordering (the 24 ordered structural checks `check_01..check_24` all run BEFORE the post-structural conformance-report byte-compare re-derivation, so any earlier structural defect surfaces with its dedicated reason; the only public emitted reason for a bundled-report disagreement or `manifest.registry_id` cross-anchor mismatch is `registry_manifest_invalid`, deliberately funnelled to avoid introducing a twenty-fifth public reason), path-traversal BEFORE exact subject-path equality, and a 36-token prohibited-registry-claim guard that scans every string value outside `scope_limitations` and `non_claims` for forbidden positive tokens covering production-PKI / certificate-authority / certification-authority / legal-identity / identity-proofing / federation / trust-federation tokens, certification, compliance, audit-readiness, regulator/auditor approval, production authorization, legal enforceability, runtime truth, transferred trust, and Gold reliance;
- a 48-numbered-exercise regression test (`tests/test_silver_registry_lite_v0_3_7.sh`) covering 4 positive-path exercises (PP1..PP4), 24 canonical verifier-reason exercises (case01..case24) plus 11 duplicate `registry_manifest_invalid` exercises (dup01..dup11) that share the structural reason `registry_manifest_invalid` to confirm reason stability under multiple distinct mutations (subject [0] / [1] absolute path, subject [0] / [1] traversal, subject [0] file missing, subject [0] `size_bytes` mismatch, subject [0] `sha256` mismatch, wrong subject count, subject [0] role wrong, `registry_id` cross-anchor mismatch, and the non-masking post-structural conformance-report disagreement), 6 runner-only refusal exercises (ro1, ro2, ro2b, ro3..ro5; ro2 and ro2b both exercise `runner_input_path_forbidden` for absolute and parent-traversal inputs respectively), 1 runner-relay-of-verifier exercise (rel01) explicitly asserting NO sixth wrapper code, 1 taxonomy gate (TG1) with an EMPTY allow-list enforcing that every reason-shaped token in v0.3.7 source and v0.3.7-anchored sections of the cross-version docs belongs to the 24-reason verifier set ∪ 5 runner-only set (no escape-hatch allow-list remains), and 1 scoped sha256 snapshot (SS) over the committed v0.3.7 source paths.

The registry-lite package is unsigned; v0.3.7 ships local hash anchors only. The registry is a hand-authored local declaration of Silver entity roles and optional trust relationships; v0.3.7 does not approve, audit, or certify the registry, does not constitute a production PKI, certificate authority, certification authority, legal identity registry, identity-proofing record, federation registry, or trust federation, and does not re-evaluate any specific upstream Silver evidence against the registry in this release.

> v0.3.7 records that a package owner has hand-authored a structured local registry of Silver entity roles whose canonical-JSON byte image satisfies 24 ordered structural checks, that the registry is internally consistent under closed enums for role / status / scope / role-specific / trust-relationship / version-binding fields, and that the bundled conformance report can be re-derived byte-for-byte from the registry body alone. It does not assert that any entity in the registry is operated by a legally-identified party, does not certify the registry, does not constitute a production trust registry, does not transfer reliance to any downstream party, and does not re-evaluate any specific upstream Silver evidence against the registry.

A Silver registry-lite package is not a Gold certificate, regulator action, auditor finding, third-party endorsement, production PKI, certificate authority, certification authority, identity-proofing record, federation registry, trust federation, legal identity registry, transfer of reliance, or authorization for production reuse.

Run and verify the v0.3.7 registry lite locally:
...
ProofRail v0.3.7 does not claim:
...
- The v0.2.6 evidence source adapter descriptors are static declarations only; they do not integrate with any real product, do not certify their declared sources, and do not assert that a declared source actually behaves as described. 
